...
### หมุดแผนที่บน Supabase

เมื่อกำหนด Supabase แล้ว หน้า `#/staff` จะบันทึกเลเยอร์ หมุด ชื่อ รายละเอียด ประเภท สถานะ สี พิกัด และรูปภาพลง Supabase ผ่าน Edge Function โดยตรง:

```bash
supabase db push
supabase secrets set ADMIN_PASSWORD_HASH=<SHA-256 ของรหัสผ่านเจ้าหน้าที่>
supabase functions deploy staff-map-markers
```

รูปภาพอยู่ใน bucket ส่วนตัว `map-marker-images` และหน้าแผนที่จะได้รับ URL ชั่วคราวจาก Edge Function ส่วนโหมดที่ไม่ได้กำหนด Supabase จะใช้ localStorage สำหรับทดสอบเท่านั้น
